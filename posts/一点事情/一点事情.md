...
//永久
pnpm install <package> --registry=https://registry.npmmirror.com
//临时
```
...
![1777569358261](1777569358261.png)

第三种属于下面的变体：standalone
...
对于使用docker-compose构建的应用

```bash 
docker compose ls      #查看所有使用docker compose构建的应用
```

一般来收config files会写出这个compose文件的路径，但是并非绝对

有时只会显示![1779346273897](1779346273897.png)

可以进对应容器：

```bash
docker inspect <容器名> | grep -i com.docker.compose.project.config_files
#文件名，和上面显示的应该是一样的
docker inspect <容器名> | grep -i com.docker.compose.project.working_dir
#路径，和上面拼接即可
```

如果你不知道编排中的某个玩意儿是用于干什么的：

```
docker logs <compose名> | grep "容器名"
```

将云端文件下载到本地（跨平台可用）

```powershell
scp root@IP:/root/xxxx 本地路径
```
